...
export class DialogAPI {
   ...
   */
  selectWorkspace(options?: { prompt?: string; showRecent?: boolean }): Promise<Workspace | null> {
   ...
   */
  selectProject(workspace: Workspace, projects: string[]): Promise<string | null> {
    return new Promise(resolve => {
      this.setState({
        type: 'select-project',
        props: {
...
export function DialogAPIProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <DialogAPIContext.Provider value={dialogAPI}>
      <DialogStateContext.Provider value={dialogState}>{children}</DialogStateContext.Provider>
