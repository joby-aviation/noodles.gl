...
describe('mergeTableSchemas', () => {
  it('retains target-only columns and appends incoming-only columns', () => {
    const current = {
      columns: [
        { id: 'name', name: 'name', type: 'string' as const, defaultValue: '' },
        { id: 'local', name: 'local', type: 'number' as const, defaultValue: 0 },
      ],
    }
    const incoming = {
      columns: [
        { id: 'name', name: 'name', type: 'string' as const, defaultValue: '' },
        { id: 'remote', name: 'remote', type: 'string' as const, defaultValue: 'new' },
      ],
    }

    const result = mergeTableSchemas(current, incoming, [{ name: 'Alice', local: 42 }])

    expect(result.schema.columns.map(column => column.name)).toEqual(['name', 'local', 'remote'])
    expect(result.data).toEqual([{ name: 'Alice', local: 42, remote: 'new' }])
    expect(result.warnings).toEqual([])
  })

  it('propagates ID-based renames without replacing target values', () => {
    const current = {
      columns: [{ id: 'person', name: 'name', type: 'string' as const, defaultValue: '' }],
    }
    const incoming = {
      columns: [{ id: 'person', name: 'display_name', type: 'string' as const, defaultValue: '' }],
    }

    const result = mergeTableSchemas(current, incoming, [{ name: 'Child value' }])

    expect(result.schema).toEqual(incoming)
    expect(result.data).toEqual([{ display_name: 'Child value' }])
    expect(result.warnings).toEqual([])
  })

  it('preserves renamed values when connected data arrives before its schema', () => {
    const current = {
      columns: [{ id: 'person', name: 'name', type: 'string' as const, defaultValue: '' }],
    }
    const incoming = {
    ...
    const result = mergeTableSchemas(current, incoming, [{ display_name: 'Source value' }])

    expect(result.schema).toEqual(incoming)
    expect(result.data).toEqual([{ display_name: 'Source value' }])
    expect(result.warnings).toEqual([])
  })

  it('matches by name when the target schema has no ID', () => {
    const current = {
      columns: [{ name: 'name', type: 'string' as const, defaultValue: '' }],
    }
    const incoming = {
      columns: [{ id: 'source-id', name: 'name', type: 'string' as const, defaultValue: 'new' }],
    }

    const result = mergeTableSchemas(current, incoming, [{ name: 'Child value' }])

    expect(result.schema).toEqual(incoming)
    expect(result.data).toEqual([{ name: 'Child value' }])
    expect(result.warnings).toEqual([])
  })

  it('skips an invalidating update while applying safe additions', () => {
    const current = {
    ...
    const result = mergeTableSchemas(current, incoming, [{ value: 'keep me' }])

    expect(result.schema.columns).toEqual([
      current.columns[0],
      { id: 'added', name: 'added', type: 'boolean', defaultValue: true },
    ])
    expect(result.data).toEqual([{ value: 'keep me', added: true }])
    expect(result.warnings[0]).toContain('invalidate 1 existing value')
  })

  it('applies valid constraints and skips constraints that invalidate target values', () => {
    const current = {
      columns: [{ id: 'value', name: 'value', type: 'number' as const, defaultValue: 0 }],
    }
    const safeIncoming = {
      columns: [
        {
    ...
    const safeResult = mergeTableSchemas(current, safeIncoming, [{ value: 10 }])
    const unsafeResult = mergeTableSchemas(current, unsafeIncoming, [{ value: 10 }])

    expect(safeResult.schema).toEqual(safeIncoming)
    expect(safeResult.warnings).toEqual([])
    expect(unsafeResult.schema).toEqual(current)
    expect(unsafeResult.data).toEqual([{ value: 10 }])
    expect(unsafeResult.warnings[0]).toContain('invalidate 1 existing value')
  })

  it('validates data that already uses an incoming renamed key', () => {
    const current = {
      columns: [{ id: 'value', name: 'old_value', type: 'string' as const, defaultValue: '' }],
    }
    const incoming = {
      columns: [{ id: 'value', name: 'new_value', type: 'number' as const, defaultValue: 0 }],
    }

    const result = mergeTableSchemas(current, incoming, [{ new_value: 'keep me' }])

    expect(result.schema).toEqual(current)
    expect(result.data).toEqual([{ old_value: 'keep me' }])
    expect(result.warnings[0]).toContain('invalidate 1 existing value')
  })

  it('skips a rename that collides with a target-owned column', () => {
    const current = {
      columns: [
        { id: 'first', name: 'first', type: 'string' as const, defaultValue: '' },
        { id: 'second', name: 'second', type: 'string' as const, defaultValue: '' },
      ],
    }
    const incoming = {
      columns: [{ id: 'first', name: 'second', type: 'string' as const, defaultValue: '' }],
    }

    const result = mergeTableSchemas(current, incoming, [{ first: 'A', second: 'B' }])

    expect(result.schema).toEqual(current)
    expect(result.data).toEqual([{ first: 'A', second: 'B' }])
    expect(result.warnings[0]).toContain('already in use')
  })

  it('does not merge a fresh ID into a target column with the same name', () => {
    const current = {
      columns: [{ id: 'original', name: 'name', type: 'string' as const, defaultValue: '' }],
    }
    const incoming = {
      columns: [{ id: 'replacement', name: 'name', type: 'number' as const, defaultValue: 0 }],
    }
