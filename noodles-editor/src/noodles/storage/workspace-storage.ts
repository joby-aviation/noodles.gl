...
import { getOPFSRoot, readFileFromDirectory, writeFileToDirectory } from '../utils/filesystem'
...
export async function listProjects(workspace: Workspace): Promise<string[]> {
  switch (workspace.type) {
    case 'folder': {
      ...
      // @ts-expect-error - entries() exists but TypeScript lib definitions may be outdated
 ...
 */
export async function deleteProject(workspace: Workspace, projectName: string): Promise<void> {
  // Check read-only
  if (workspace.type === 'examples') {
 ...
 */
export async function projectExists(workspace: Workspace, projectName: string): Promise<boolean> {
...
export async function getRecentProjects(
  ...
  return projectsWithTimestamps.sort((a, b) => b.lastModified - a.lastModified).slice(0, limit)
}
