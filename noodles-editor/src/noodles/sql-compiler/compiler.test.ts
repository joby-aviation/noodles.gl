...
describe('isCompilable', () => {
  it('returns true for registered operator types', () => {
    ...
    expect(isCompilable(makeNode('/f', 'FilterOp', { columnName: 'x', condition: 'equals', value: '5' }))).toBe(true)
...
describe('compile', () => {
  ...
  it('compiles FileOp → FilterOp chain', () => {
    const nodes = [
      ...
      makeNode('/filter', 'FilterOp', { columnName: 'age', condition: 'greater than', value: '30' }, ['/data']),
    ]
    const result = compile(nodes)
    expect(result.sql).toContain('read_csv_auto($1')
  ...
  it('compiles FileOp → FilterOp → SortOp chain', () => {
    const nodes = [
      ...
      makeNode('/filter', 'FilterOp', { columnName: 'score', condition: 'greater than', value: '50' }, ['/data']),
  ...
  it('compiles CastOp', () => {
    const nodes = [
      makeNode('/data', 'File', { url: 'x.csv', format: 'csv' }),
      makeNode('/cast', 'Cast', { column: 'age', targetType: 'INTEGER', outputColumn: 'age_int' }, ['/data']),
    ]
    const result = compile(nodes)
  ...
  it('compiles CoalesceOp', () => {
    const nodes = [
      makeNode('/data', 'File', { url: 'x.csv', format: 'csv' }),
      makeNode('/coal', 'Coalesce', { columns: 'gps_lat,manual_lat', outputColumn: 'latitude' }, ['/data']),
    ]
    const result = compile(nodes)
  ...
  it('compiles FilterOp with contains condition', () => {
    const nodes = [
      makeNode('/data', 'File', { url: 'x.csv', format: 'csv' }),
      makeNode('/filter', 'FilterOp', { columnName: 'name', condition: 'contains', value: 'air' }, ['/data']),
    ]
    const result = compile(nodes)
  ...
  it('assigns correct CTE aliases from operator IDs', () => {
    const nodes = [
      ...
      makeNode('/processing/filter-1', 'FilterOp', { columnName: 'x', condition: 'equals', value: '1' }, ['/my-data-source']),
    ]
    const result = compile(nodes)
  ...
  it('handles SQL reserved words in operator IDs', () => {
    const nodes = [
      makeNode('/select', 'File', { url: 'x.csv', format: 'csv' }),
    ]
    const result = compile(nodes)
...
describe('collectSubgraph', () => {
  it('collects a linear chain in topological order', () => {
    const nodes = new Map<string, CompilableNode>([
      ...
      ['/filter', makeNode('/filter', 'FilterOp', { columnName: 'x', condition: 'equals', value: '1' }, ['/data'])],
  ...
  it('collects diamond-shaped subgraphs correctly', () => {
    const nodes = new Map<string, CompilableNode>([
      ...
      ['/join', makeNode('/join', 'Join', { leftKey: 'code', rightKey: 'id', joinType: 'inner' }, ['/data', '/airports'])],
    ])
