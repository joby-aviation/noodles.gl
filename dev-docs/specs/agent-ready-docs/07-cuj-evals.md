...
| `code-refs-containers` | 3 | "Compute a derived column with a CodeOp, wire a threshold via an `op()` reference, and group the pipeline into a container" (added in phase 0 — no other task touches containers, and none requires *writing* reactive references; built in step 5) | validates; CodeOp present whose `op()` reference resolves; container children carry correctly path-prefixed ids; renders |
