...
Tiers are enforced by environment construction, not by asking the agent to abstain: T0–T2 get no MCP config and no `skills/` directory (the runner strips them from the checkout as needed); T3+ get the pieces added. The T5 environment must also **grant agent write access explicitly** (set the per-origin consent state, or launch with the session override, from 05's consent model) — 05 gates write tools default-OFF, and `live-drive` requires writes; without this step T5 fails by design with `NotAllowedError` and the failure reads as a docs problem.
