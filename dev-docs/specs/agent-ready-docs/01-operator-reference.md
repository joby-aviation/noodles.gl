...
2. **Normative voice** (summary, tables, behavior statements): present tense, third person, MSDN register ("Returns the…", "If *data* is empty, the output is `undefined`"). RFC 2119 keywords (MUST/SHOULD/MAY) only where behavior is contractual across versions — serialization format, clamping rules — used the way RFC 7946 uses them: sparingly, capitalized.
3. **Remarks voice** (Raymond Chen): first person plural allowed; answer "why is it like this", "what will bite you", "what did we try first". Every Remarks section must contain at least one thing not derivable from the field table. No marketing adjectives.
4. **Examples** — every example must load; reference real projects under `noodles-editor/src/examples/` where possible.
5. A short before/after sample of each voice.
